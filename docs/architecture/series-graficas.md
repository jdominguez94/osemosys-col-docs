...
Esta página documenta la parte de desarrollo/administración de la configuración global de series de gráficas: las tablas involucradas, cómo se relacionan entre sí, el flujo de datos desde la base de datos hasta la gráfica, cómo agregar nuevos tipos de gráfica al sistema, los endpoints de API disponibles y el mapa de archivos clave.
...
!!! warning "Restricciones"
    Restricción única: `(tipo, agrupar_por, series_code)` — no puede haber dos filas con el mismo código de serie para el mismo par tipo+agrupación. Índice compuesto: `(tipo, agrupar_por, sort_index)` para consultas ordenadas rápidas.
...
## Flujo de datos: cómo `chart_series_config` afecta las gráficas
...
7. Si hay filas en `chart_series_config` para ese tipo+agrupación: filtra series ocultas (`hidden=true`), reordena por `sort_index` y aplica color/nombre override. Si no hay filas, devuelve el orden/colores por defecto.
...
4. Reordena: series con fila config primero (por `sort_index`, luego índice original); el resto mantiene el orden original al final.
...
**Cobertura indirecta:** `build_comparison_facet_data()` llama a `build_chart_data()` por job → hereda la config.
...
El `tipo` debe existir en **tres capas** para funcionar de punta a punta: `CONFIGS` (backend) para que funcione el chart-data, el `MENU` del `ChartSelector` (frontend) para que aparezca en el selector, y opcionalmente `chart_series_config` para colores/nombres/orden admin.
...
La población es **no destructiva**: solo inserta filas faltantes; no modifica las existentes. (El flujo equivalente desde la interfaz de administración — pestaña "Series por gráfica" en Reportes — está documentado para el usuario final en la guía de uso de la aplicación.)
