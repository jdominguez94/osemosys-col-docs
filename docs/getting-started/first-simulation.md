...
- Si necesitas crear uno nuevo, hay **dos formas de cargar los datos de entrada**:
...
!!! example "📸 Captura pendiente"
    Pantalla de carga del archivo ClicSAND (`.xlsx`).

    <!-- ![Carga ClicSAND](../assets/screenshots/first-simulation/02a-import-excel.png) -->
...
!!! example "📸 Captura pendiente"
    ...
    <!-- ![Lanzar simulación](../assets/screenshots/first-simulation/03-launch.png) -->
...
!!! example "📸 Captura pendiente"
    Pantalla con el estado del job (en cola / en ejecución / finalizado).

    <!-- ![Estado del job](../assets/screenshots/first-simulation/04-job-status.png) -->

## 5. Abrir los resultados

Cuando el job termina exitosamente, ábrelo desde la lista de simulaciones para entrar a la página de resultados. Ahí encontrarás la **planeación de largo plazo** resultante de la simulación:
...
Los resultados de la planeación de largo plazo quedan disponibles de dos formas simultáneas:

- **Descarga en CSV**: desde la página de resultados puedes exportar los valores de salida (`GET /simulations/{job_id}/output-values/export`) o una gráfica puntual en CSV/Excel/PNG/SVG.
- **Persistencia en PostgreSQL**: todos los resultados quedan guardados en la base de datos del stack (tabla `osemosys_output_param_value`, entre otras), por lo que puedes volver a consultarlos, compararlos con otros escenarios o generar reportes más adelante sin tener que repetir la simulación.
...
- [Carga de datos Excel/SAND](../user-guide/carga-excel-sand.md) para el detalle de la carga vía ClicSAND.
