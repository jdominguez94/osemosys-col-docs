...
Este modo sirve para validar rápidamente un archivo de datos sin pasar por el proceso de creación de un escenario, para reproducir o comparar una corrida hecha originalmente fuera de la aplicación (por ejemplo, en una hoja de cálculo), y para pruebas puntuales donde no necesitas conservar el escenario para uso futuro.
...
El archivo debe seguir el formato SAND esperado por la aplicación (una hoja **Parameters** con la estructura de sets y valores que usa el modelo OSeMOSYS). Ver [Archivos SAND](../examples/sand-files.md) para la ubicación de archivos SAND de referencia y un ejemplo de su estructura.
...
No hay **persistencia del escenario como reutilizable**. Los datos de entrada no quedan guardados como escenario, así que si necesitas volver a simular con los mismos datos, deberás volver a subir el archivo (o importarlo como escenario en base de datos).

## Visualización de resultados

Una vez que la simulación desde Excel/SAND termina, los resultados se visualizan exactamente igual que los de un escenario en base de datos, con el mismo selector de gráficas, los mismos tipos de vista y las mismas opciones de exportación. Ver [Visualizaciones y reportes](visualizaciones.md).
...
Revisa [Archivos SAND](../examples/sand-files.md) para ver dónde están los archivos SAND de referencia, las políticas que representan y un ejemplo de su estructura. Consulta [Visualizaciones y reportes](visualizaciones.md) para explorar los resultados. Y si luego decides importar estos datos como un escenario persistente, ve a [Escenarios y catálogos](escenarios.md).
