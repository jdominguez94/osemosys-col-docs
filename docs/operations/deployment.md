...
- Archivos de entorno preparados: `.env` (raíz), `backend/.env` y, si aplica, `frontend/.env`.
...
| `simulation-worker` (Celery) | build `./backend` | sin puerto expuesto | — |
...
- se crea un entorno virtual `.venv` e instala `backend/requirements.txt`;
- se genera `backend/.env.local` (si no existe), con `DATABASE_URL=sqlite:///./tmp/local/osemosys_local.db` y `SIMULATION_MODE=sync`;
- se inicializa SQLite local y un seed mínimo (`seed/seed123`);
- la API queda disponible en `http://localhost:8000` (`/docs`); usa `.\scripts\run-local-api.ps1 -Port 8010` para cambiar el puerto.

## Scripts disponibles en `scripts/`

| Script | Propósito |
|---|---|
| `deploy-local.sh` | Despliegue local/manual: copia `.env`/`backend/.env` desde los `.example` y levanta el stack (usado con `SECRET_KEY` y `APP_PASSWORD` por variable de entorno). |
